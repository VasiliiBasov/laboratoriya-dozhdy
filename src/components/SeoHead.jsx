...
const SITE_URL = 'https://thegreenstone.ru';
const SITE_NAME = 'TheGreenStone — Лаборатория дождя';
...
const SeoHead = ({
    ...
    // path — относительный путь БЕЗ домена (например "/services").
    // Полный URL соберётся автоматически как SITE_URL + path.
    path = '/',
    // h1 — обязателен. Это «главный заголовок страницы» по SEO.
    // Рендерим visually-hidden, чтобы внешний вид страниц не менялся.
    h1,
    // h1Hidden — если true (по умолчанию), H1 визуально скрыт (visually-hidden).
    // Если false, H1 будет видимым — используйте это, когда дизайн страницы
    // предполагает явный заголовок.
    h1Hidden = true,
    // ISO-дата обновления страницы (YYYY-MM-DD). Если не указана — сегодня.
    dateModified,
    // Автор страницы (по умолчанию — SITE_NAME).
    author = SITE_NAME,
}) => {
    const fullTitle = title ? `${title} — ${SITE_NAME}` : SITE_NAME;
    const fullUrl = `${SITE_URL}${path}`;
    // Если dateModified не передан — берём сегодня в ISO-формате (YYYY-MM-DD).
    const resolvedDate =
        dateModified || new Date().toISOString().slice(0, 10);

    // JSON-LD WebPage — расширенная микроразметка (п. «Семантическая разметка»
    // и п. «Расширенные данные» из отчёта beget). Невидимо для пользователя.
    const webPageJsonLd = {
    ...
    return (
        <>
            ...
            <Helmet>
                ...
                <meta name="generator" content="TheGreenStone" />
